...
for version in ${VERSIONS}; do
    ...
    if [[ "${version}" != "8" ]]; then
        eval update_generic_linux "\${CORRETTO_${version}_GENERIC_LINUX}" "${version}"
        eval update_amazon_linux "\${CORRETTO_${version}_AMAZON_LINUX}" "${version}"
    fi
done
...
for version in ${VERSIONS}; do
    ...
    eval verify_update "${version}" "\${CORRETTO_${version}_GENERIC_LINUX}|\${CORRETTO_${version}_AMAZON_LINUX}|\${CORRETTO_${version}_ALPINE}"
done
